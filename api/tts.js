...
const MAX_CHARS = 2600; // Internal Gemini chunk size.
const MAX_TOTAL_CHARS = 5000; // User-facing text limit.
const MAX_RETRIES = 2;
...
function pcmWav(pcm, sampleRate = 24000, channels = 1, bits = 16) {
  ...
  out.writeUInt32LE(sampleRate * channels * bits / 8, o); o += 4;
...
function processPcm(input, speed = 1, volume = 1) {
  ...
  for (let i = 0; i < outCount; i++) {
    ...
    out.writeInt16LE(Math.max(-32768, Math.min(32767, Math.round(sample * gain))), i * 2);
  }
  return out;
}
...
function cleanErrorMessage(status, rawMessage) {
  ...
  if (status === 429) return 'Gemini AI Voice limit ရောက်နေပါတယ်။ ခဏစောင့်ပြီး ထပ်စမ်းပါ။';
  if (status === 401 || status === 403) return 'Gemini API Key / project permission ပြဿနာရှိပါတယ်။';
...
async function generateChunk(apiKey, text, voice, style, language) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      error.status = r.status; error.rawMessage = rawMessage; lastError = error;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (text.length > MAX_TOTAL_CHARS) return res.status(400).json({ error: 'စာသားအရှည်သည် 5,000 characters ထက် မကျော်ရပါ။' });
    const selectedLanguage = language === 'en' ? 'en' : 'my';
    const chunks = chunkText(text);
    const audioChunks = [];
    for (const chunk of chunks) audioChunks.push(await generateChunk(apiKey, chunk, voice, style, selectedLanguage));
    ...
    return res.status(200).json({
      mime:'audio/wav', filename:'gemini-ai-voice.wav', audio:wav.toString('base64'), chunks:chunks.length,
      model:MODEL, provider:'gemini', language:selectedLanguage === 'en' ? 'en-US' : 'my',
      speed:Math.max(0.7, Math.min(1.3, Number(speed) || 1)), volume:normalizedVolume, fallbackAvailable:true
    });
  } catch (e) {
    const status = Number(e?.status) || 500;
    const clientStatus = status === 429 ? 429 : (status >= 500 ? 503 : status === 400 ? 400 : 500);
    return res.status(clientStatus).json({error:e?.message || 'TTS generation failed.',provider:'gemini',fallback:'browser',retryable:clientStatus === 429 || clientStatus === 503});
  }
}
